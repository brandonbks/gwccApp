...
def outputpage():
    ...
    #return render_template("home-page.html")
    return render_template("output-page.html", eventInfo=eventInfo, cost=cost, margin=margin, eventName=eventName, eventID=eventID, marginList = json.dumps(marginList))
...
def export():
    eventInfo = transQueryEvents("SELECT * FROM EVENTS")
    for event in eventInfo:
        event["name"] = event["name"].replace(",", "/")
        event["rooms"] = event["rooms"].replace(",", "/")
    print(len(eventInfo))
    return json.dumps({"allEvents": eventInfo})


@app.route("/slider", methods=["POST"])
def slider():
    eventInfo = json.loads(request.form["eventInfo"])
    print(eventInfo)
    eventInfo["sqftPerEvent"] = int(eventInfo["sqftPerEvent"])
    eventInfo["orderedRentTotal"] = int(eventInfo["orderedRentTotal"])
    eventInfo["Attendance"] = int(eventInfo["Attendance"])
    eventInfo["totalRoomNights"] = int(eventInfo["totalRoomNights"])
    eventInfo["FB"] = int(eventInfo["FB"])

    del eventInfo["eventType"]
    del eventInfo["dateIn"]
    ...
    print(cost[0])
    return json.dumps({"updatedCost": cost[0]})
...
def regmodel():
    ...
    eventName = request.args.get("eventName")
    insertQueryEvents('INSERT INTO EVENTS (eventID, name, type, startDate, endDate, attendance, sqft, roomNights, subDate, rooms, cost, baselinePrice, isBooked) VALUES (?,?,?,?,?,?,?,?,?,?,?,?, ?)',
                    [maxId+1, eventName, eventInfo["eventType"], startDateExcel, endDateExcel, eventInfo["Attendance"], eventInfo["sqftPerEvent"], eventInfo["totalRoomNights"], subDateExcel, request.args.get("rooms"), float(cost), float(cost*(1+suggMargin)), 0])
    return redirect(url_for('outputpage', eventInfo=eventInfo, cost=cost, margin=suggMargin, eventName=eventName, eventID=maxId+1, cluster=cluster, demand=demand))
...
def book(): 
    ...
    eventId = request.form['eventId']
    #eventInfo = transQueryEvents("SELECT * FROM EVENTS WHERE eventId = ?", [eventId])[0]
    insertQueryEvents("UPDATE EVENTS SET isBooked=? WHERE eventId=?", [1, eventId])
...
def margin(): 
    ...
    insertQueryEvents("UPDATE EVENTS SET baselinePrice=? WHERE eventId=?", [baseline, eventId])
    return json.dumps({"status": "success"})
